...
export default function PostPage() {
  ...
  useEffect(() => {
    if (post && post.userId) {
      const getUser = async () => {
        try {
          const res = await fetch(`https://blogbreeze-2.onrender.com/api/user/${post.userId}`);
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        setLoading(true);
        const res = await fetch(`https://blogbreeze-2.onrender.com/api/post/getposts?slug=${postSlug}`);
        const data = await res.json();
        if (!res.ok) {
          setError(true);
          setLoading(false);
  ...
  useEffect(() => {
    const fetchRecentPosts = async () => {
      try {
        const res = await fetch(`https://blogbreeze-2.onrender.com/api/post/getposts?limit=3`);
        const data = await res.json();
        if (res.ok) {
  ...
  useEffect(() => {
    if (post && post.userId) {
      const fetchUserPosts = async () => {
        try {
          const res = await fetch(`https://blogbreeze-2.onrender.com/api/post/getposts?userId=${post.userId}&limit=3`);
          const data = await res.json();
          if (res.ok) {
