...
export default function DashPosts() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const res = await fetch(`https://blogbreeze-nj8u.onrender.com/api/post/getposts?userId=${currentUser._id}`);
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch( `https://blogbreeze-nj8u.onrender.com/api/post/getposts?userId=${currentUser._id}&startIndex=${startIndex}`);
      const data = await res.json();
      if (res.ok) {
